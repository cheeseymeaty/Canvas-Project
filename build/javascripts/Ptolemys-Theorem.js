...
const settings = {
	...
	// How smooth the mouse movement is
	mouseSmoothness: 5,

	// How smooth the dot movement is
	dotSmoothness: 5
};
...
const vertexes = (() => {
	...
	const side = Math.sin(60*(Math.PI/180))*circle.radius;

	// The distance from the center of the circle to the base of the triangle
	const offset = Math.sin(30*(Math.PI/180))*circle.radius;
...
function animate() {
	...
	const lines = [
		// Top
		{ distance: Math.hypot(vertexes[0].x-point.smooth.x, vertexes[0].y-point.smooth.y), index: 0 },

		// Left
		{ distance: Math.hypot(vertexes[1].x-point.smooth.x, vertexes[1].y-point.smooth.y), index: 1 },

		// Right
		{ distance: Math.hypot(vertexes[2].x-point.smooth.x, vertexes[2].y-point.smooth.y), index: 2 }
	];
	...
	for (let i = 0; i < vertexes.length; i++) {
		...
		C.lineTo(circle.x+vertexes[(i+1)%vertexes.length].x, circle.y+vertexes[(i+1)%vertexes.length].y);
		C.stroke();
	}
	...
	for (let i = 0, total = 0; i < shorterLines.length; (total += shorterLines[i].distance) && i++) {
		...
		C.beginPath();
		C.strokeText(Math.round(line.distance), circle.x+(overall+settings.style.text.spaceFromShortest)*(i === 0 ? -1 : 1), y);
		C.fillText(Math.round(line.distance), circle.x+(overall+settings.style.text.spaceFromShortest)*(i === 0 ? -1 : 1), y);
	}
	...
	mouse.smooth.x += (mouse.real.x-mouse.smooth.x)/settings.mouseSmoothness;
	mouse.smooth.y += (mouse.real.y-mouse.smooth.y)/settings.mouseSmoothness;
	...
	point.smooth.x += (point.real.x-point.smooth.x)/settings.dotSmoothness;
	point.smooth.y += (point.real.y-point.smooth.y)/settings.dotSmoothness;
...
onmousemove = E => {
	...
	mouse.inCircle = Math.hypot(mouse.smooth.x-circle.x, mouse.smooth.y-circle.y) <= circle.radius;
};
